...
public class AdminController {
    ...
    public ResponseEntity<?> saveWasenderApiKey(@RequestBody WasenderApiKeyRequest request, HttpSession session) {
        try {
            String apiKey = request.getWasenderApiKey();
            
            if (apiKey == null || apiKey.trim().isEmpty()) {
                Map<String, String> errorResponse = new HashMap<>();
                errorResponse.put("error", "WASender PAT cannot be empty");
                return ResponseEntity.badRequest().body(errorResponse);
            }
            
            apiKey = apiKey.trim();
            
            // Save to global config first (this always succeeds)
            wasenderConfigService.saveApiKey(apiKey);
            
            // Always set session attribute immediately (for immediate availability)
            session.setAttribute("wasenderApiKey", apiKey);
            
            // Also update the current user's WASender API key in database (non-blocking)
            String userIdStr = (String) session.getAttribute("userId");
            if (userIdStr != null) {
                try {
                    UUID userId = UUID.fromString(userIdStr);
                    // Try to update with validation (updates subscription info if valid)
                    try {
                        userService.updateWasenderApiKey(userId, apiKey);
                    } catch (Exception validationEx) {
                        // If validation fails, still save the PAT but without subscription info
                        try {
                            userService.updateWasenderApiKeyWithoutValidation(userId, apiKey);
                        } catch (Exception saveEx) {
                            log.warn("Could not save PAT to user record: {}", saveEx.getMessage());
                            // Continue anyway - session attribute is already set
                        }
                    }
                } catch (Exception e) {
                    log.warn("Could not update user WASender API key in database: {}", e.getMessage());
                    // Continue anyway - session attribute is already set
                }
            }
    ...
    public ResponseEntity<?> regenerateSiteApiKey(@PathVariable(name = "siteId") String siteId) {
        try {
            com.clapgrow.notification.api.dto.SiteRegistrationResponse response = siteService.regenerateApiKey(java.util.UUID.fromString(siteId));
            return ResponseEntity.ok(response);
        } catch (IllegalArgumentException e) {
            Map<String, String> errorResponse = new HashMap<>();
            errorResponse.put("error", e.getMessage());
    ...
    public ResponseEntity<Map<String, Object>> getAllSessions(HttpSession session) {
        ...
        if (apiKey == null) {
            Map<String, Object> error = new HashMap<>();
            error.put("success", false);
            error.put("error", "WASender API key is not configured. Please go to Admin > Sites and save your WASender PAT.");
            error.put("errorCode", "API_KEY_REQUIRED");
            error.put("requiresApiKey", true);
            return ResponseEntity.status(428).body(error); // 428 Precondition Required
        }

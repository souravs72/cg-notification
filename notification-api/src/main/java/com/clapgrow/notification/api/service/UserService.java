...
public class UserService {
    ...
    /**
     * Update WASender API key without validation (saves PAT even if validation fails)
     * Use this when you want to save the PAT regardless of validation status
     */
    @Transactional
    public User updateWasenderApiKeyWithoutValidation(UUID userId, String wasenderApiKey) {
        ...
        user.setWasenderApiKey(wasenderApiKey.trim());
        user.setUpdatedBy("SYSTEM");
        // Don't update subscription info - leave it as is

        user = userRepository.save(user);
        log.info("Updated WASender API key for user: {} (without validation)", user.getEmail());

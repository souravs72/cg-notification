...
public class WasenderQRService {
    ...
    public Map<String, Object> getAllSessions(String apiKey) {
        try {
            if (apiKey == null || apiKey.trim().isEmpty()) {
                log.error("WASender API key is null or empty");
                throw new IllegalStateException("WASender API key is required");
            }
            
            // Log masked PAT for debugging
            String maskedKey = apiKey.length() > 15 
                ? apiKey.substring(0, 10) + "..." + apiKey.substring(apiKey.length() - 5)
                : "***";
            log.info("Fetching all WhatsApp sessions with PAT (length: {}, masked: {})", apiKey.length(), maskedKey);
            
            WebClient webClient = webClientBuilder.build();
            
            String sessionsUrl = wasenderBaseUrl + "/whatsapp-sessions";
            log.debug("Calling WASender API: {}", sessionsUrl);
            ...
            // Log PAT info for debugging (masked)
            String maskedKey = apiKey != null && apiKey.length() > 15 
                ? apiKey.substring(0, 10) + "..." + apiKey.substring(apiKey.length() - 5)
                : (apiKey != null ? "***" : "null");
            log.error("PAT used (masked): {}, length: {}", maskedKey, apiKey != null ? apiKey.length() : 0);
